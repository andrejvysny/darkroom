...
//!
//! Also doubles as a corpus survey (per-file failures are reported, never abort the run) — point it
//! at a folder of `.heic`/`.hif` files (e.g. the Nokia HEIF conformance suite) to empirically map
//! which profiles the linked libheif/libde265 can decode (bit depth, chroma, nclx).
//!
//! Per file it prints: container/handle dims + bit depth, nclx codes (expect primaries=BT.2020,
//! transfer=PQ for Canon), decode timing, code-value percentiles, embedded thumbnail items, Exif
//! parse (model/ISO/shutter/orientation), a whole-path `develop_linear` check (expected to FAIL for
//! non-`.hif` extensions — only Canon `.hif` is whitelisted), and the PORTRAIT check: decoded dims
//! with vs without container transformations — if a portrait file shows identical dims both ways
//! but carries EXIF orientation 6/8, Canon relies on the EXIF tag and `heif.rs` must apply
//! `.oriented()` (risk R4 in the plan).
...
#[cfg(not(windows))]
fn probe(lib: &libheif_rs::LibHeif, f: &std::path::Path) -> anyhow::Result<()> {
    ...
    let plane = planes
        .interleaved
        .ok_or_else(|| anyhow::anyhow!("decoded image has no interleaved RGB plane"))?;
    println!(
    ...
    for row in (0..plane.height as usize).step_by(37) {
        ...
        let Some(row_bytes) = plane.data.get(start..start + plane.width as usize * 6) else {
            println!("  (sample read out of bounds — skipping percentiles)");
            break;
        };
    ...
    if let Some(rplane) = rp.interleaved {
        println!(
    ...
    // Whole shipping path: develop_linear → linear ProPhoto stats. Only `.hif` dispatches to the
    // HEIF decoder (a `.heic` corpus file lands on the rawler path and fails — expected).
    let t = Instant::now();
...
fn main() -> anyhow::Result<()> {
        ...
        std::fs::read_dir(&path)?
            ...
            .filter(|p| {
                p.extension()
                    .and_then(|s| s.to_str())
                    .map(|s| {
                        s.eq_ignore_ascii_case("hif")
                            || s.eq_ignore_ascii_case("heic")
                            || s.eq_ignore_ascii_case("heif")
                    })
                    .unwrap_or(false)
            })
            .collect()
    ...
    files.sort();
    if files.is_empty() {
        eprintln!("no .hif/.heic files under {arg} — copy fixtures there and re-run");
        return Ok(());
    }
    ...
    println!(
        "summary: {ok} probed OK, {failed} failed, of {} files",
        files.len()
    );
    Ok(())
}
